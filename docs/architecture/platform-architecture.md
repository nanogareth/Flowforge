...
**Status:** v3.2 — clarifications on hook generation scope
**Date:** 2026-02-15
  ...
  "failures": [
    {
      ...
      "file": "tests/test_pipeline.py:38"
    }
  ]
}
```
...
### Hook Generation Quick Reference

Not all hooks described in this section are generated by the FlowForge template system. This table clarifies the source of each hook:

| Hook                        | Script                     | Generated by `platform.ts`? | Wired by `settings.ts`?  | Notes                                      |
| --------------------------- | -------------------------- | --------------------------- | ------------------------ | ------------------------------------------ |
| 5.1 Block Test Execution    | `block-test-execution.sh`  | Yes                         | Yes (all workflows)      |                                            |
| 5.2 RTK Auto-Wrapper        | (global)                   | No                          | No                       | Installed globally via `rtk init --global` |
| 5.3 File Protection         | `protect-files.sh`         | Yes                         | Yes (all workflows)      |                                            |
| 5.4 Auto-Format             | `auto-format.sh`           | Yes                         | Yes (all workflows)      |                                            |
| 5.5 Typecheck After Edits   | `post-typecheck.sh`        | Yes                         | Yes (TDD workflows only) | Excluded from learning                     |
| 5.6 Codebase Mapper Regen   | `auto-remap.sh`            | Yes                         | Yes (TDD workflows only) | Async                                      |
| 5.7 Dependency Auto-Install | `auto-deps.sh`             | Yes                         | Yes (TDD workflows only) | Async                                      |
| 5.8 Session State Injection | `session-state.sh`         | Yes                         | Yes (all workflows)      |                                            |
| 5.9 Context Budget Monitor  | (prompt type)              | No                          | No                       | Design-only; not yet in template system    |
| 5.10 Emergency Handover     | `pre-compact-handover.sh`  | Yes                         | Yes (all workflows)      |                                            |
| 5.11 TDD Test Loop          | `stop-test-loop.sh`        | Yes                         | Yes (TDD workflows only) |                                            |
| 5.12 Exploration Reminder   | `post-explore-reminder.sh` | Yes                         | Yes (TDD workflows only) |                                            |

"TDD workflows" = research, feature, greenfield (not learning).
...
> **Template status:** This prompt-type Stop hook is described as part of the platform design but is NOT currently generated by the template system (`settings.ts`). It would need to be added to `buildSettings()` or configured globally. Currently only `stop-test-loop.sh` is wired to the Stop event in generated repos.
