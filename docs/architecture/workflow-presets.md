...
**Status:** v3.1 — hook activation matrix updated to match implementation
**Date:** 2026-02-13
...
The platform layer provides the infrastructure that every project needs regardless of workflow or stack. This is documented in `platform-architecture.md` and implemented in `flowforge-mobile/lib/templates/platform.ts`. Includes:

- 10 hook scripts in `.claude/hooks/` (block-test-execution, protect-files, auto-format, auto-deps, auto-remap, post-typecheck, session-state, pre-compact-handover, stop-test-loop, post-explore-reminder)
- Memory architecture skeleton (reference library, error corrections, explorations, handover directory)
- Slash commands (`/session-handover`, `/remap`, `/stack-check`, `/hook-check`)
- `tests/reports/.gitkeep` for structured test output
- Base `.gitignore`
...
The stack layer adds technology-specific configuration. Implemented in `flowforge-mobile/lib/templates/stacks/`, `settings.ts`, `devcontainer.ts`:

- **Stack config files** — `tsconfig.json` (TS stacks), `pyproject.toml` + `tests/conftest.py` (Python), `Cargo.toml` (Rust)
...
- **Devcontainer features** — language runtime features for CC web containers + `post-create.sh` tooling installs
- **Environment variables** — `TEST_RUNNER_CMD`, `TEST_REPORT_FORMAT`, `TYPE_CHECK_CMD`, `FORMAT_CMD`, `LINT_CMD` — set in `.claude/settings.json` `env`, consumed by hook scripts to stay stack-agnostic
- **`.claude/settings.json`** — generated by `buildSettings(workflow, stack)` with hook wiring (conditional on workflow) and env vars (conditional on stack)
...
| auto-deps | PostToolUse | On | On | On | **Off** |
| auto-remap | PostToolUse | On | On | On | **Off** |
| post-explore-reminder | PostToolUse | On | On | On | **Off** |
| stop-test-loop | Stop | On | On | On | **Off** |

**Rationale for Learning overrides:**

The learning workflow disables all TDD-related hooks (the bottom 5 rows). These are the hooks that enforce the test-driven development cycle. In learning mode, the goal is exploration, not shipping — enforcing TDD creates friction.
...
- **auto-deps / auto-remap / post-explore-reminder Off** — These support the TDD cycle (keeping deps installed, codebase map fresh, and explorations persisted). In learning mode, the user handles these manually when needed.
